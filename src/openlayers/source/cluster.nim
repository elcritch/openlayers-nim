...
type Cluster* = ref object of JsRoot
proc newCluster*(): Cluster {.importjs: "(new olNs_source_Cluster.default())".}

type ClusterOptions* = ref object of JsRoot

proc newClusterOptions*(): ClusterOptions {.importjs: "({})".}
proc `attributions=`*(
  options: ClusterOptions, value: JsObject
) {.importjs: "#.attributions = #".}

proc `attributions=`*(
  options: ClusterOptions, value: RootRef
) {.importjs: "#.attributions = #".}

proc `distance=`*(options: ClusterOptions, value: float) {.importjs: "#.distance = #".}
proc `minDistance=`*(
  options: ClusterOptions, value: float
) {.importjs: "#.minDistance = #".}
...
proc `createCluster=`*(
  options: ClusterOptions, value: JsObject
) {.importjs: "#.createCluster = #".}

proc `createCluster=`*(
  options: ClusterOptions, value: RootRef
) {.importjs: "#.createCluster = #".}

proc `createCluster=`*[T](
  options: ClusterOptions, value: seq[T]
) {.importjs: "#.createCluster = #".}
...
): Cluster {.importjs: "(new olNs_source_Cluster.default(#))".}
