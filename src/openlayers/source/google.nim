...
type Google* = ref object of JsRoot
proc newGoogle*(
  options: JsObject
): Google {.importjs: "(new olNs_source_Google.default(#))".}

proc getError*(self: Google): JsObject {.importjs: "#.getError()".}
proc fetchSessionToken*(
  self: Google, url: cstring, config: JsObject
): JsObject {.importjs: "#.fetchSessionToken(#, #)".}

type GoogleOptions* = ref object of JsRoot
...
proc `imageFormat=`*(
  options: GoogleOptions, value: cstring
) {.importjs: "#.imageFormat = #".}
...
proc `layerTypes=`*(
  options: GoogleOptions, value: seq[cstring]
) {.importjs: "#.layerTypes = #".}
...
proc `attributionsCollapsible=`*(
  options: GoogleOptions, value: bool
...
proc `reprojectionErrorThreshold=`*(
  options: GoogleOptions, value: float
) {.importjs: "#.reprojectionErrorThreshold = #".}

proc `tileLoadFunction=`*(
  options: GoogleOptions, value: JsObject
) {.importjs: "#.tileLoadFunction = #".}

proc `tileLoadFunction=`*(
  options: GoogleOptions, value: RootRef
) {.importjs: "#.tileLoadFunction = #".}
