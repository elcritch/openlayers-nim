...
type BingMaps* = ref object of JsRoot
proc newBingMaps*(
  options: JsObject
): BingMaps {.importjs: "(new olNs_source_BingMaps.default(#))".}

proc getApiKey*(self: BingMaps): cstring {.importjs: "#.getApiKey()".}
proc getImagerySet*(self: BingMaps): cstring {.importjs: "#.getImagerySet()".}
proc handleImageryMetadataResponse*(
  self: BingMaps, response: JsObject
) {.importjs: "#.handleImageryMetadataResponse(#)".}

proc quadKey*(
  tileCoord: JsObject
): cstring {.importjs: "olNs_source_BingMaps.quadKey(#)".}

type BingMapsOptions* = ref object of JsRoot

proc newBingMapsOptions*(): BingMapsOptions {.importjs: "({})".}
proc `cacheSize=`*(
  options: BingMapsOptions, value: float
) {.importjs: "#.cacheSize = #".}
...
proc `imagerySet=`*(
  options: BingMapsOptions, value: cstring
) {.importjs: "#.imagerySet = #".}

proc `interpolate=`*(
  options: BingMapsOptions, value: bool
...
proc `tileLoadFunction=`*(
  options: BingMapsOptions, value: JsObject
) {.importjs: "#.tileLoadFunction = #".}

proc `tileLoadFunction=`*(
  options: BingMapsOptions, value: RootRef
) {.importjs: "#.tileLoadFunction = #".}
...
proc `placeholderTiles=`*(
  options: BingMapsOptions, value: bool
) {.importjs: "#.placeholderTiles = #".}
