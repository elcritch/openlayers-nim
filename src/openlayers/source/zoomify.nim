...
type Zoomify* = ref object of JsRoot
proc newZoomify*(
  options: JsObject
): Zoomify {.importjs: "(new olNs_source_Zoomify.default(#))".}

type ZoomifyOptions* = ref object of JsRoot

proc newZoomifyOptions*(): ZoomifyOptions {.importjs: "({})".}
proc `attributions=`*(
  options: ZoomifyOptions, value: JsObject
) {.importjs: "#.attributions = #".}

proc `attributions=`*(
  options: ZoomifyOptions, value: RootRef
) {.importjs: "#.attributions = #".}

proc `cacheSize=`*(
  options: ZoomifyOptions, value: float
) {.importjs: "#.cacheSize = #".}

proc `crossOrigin=`*(
  options: ZoomifyOptions, value: cstring
...
proc `interpolate=`*(
  options: ZoomifyOptions, value: bool
...
proc `tierSizeCalculation=`*(
  options: ZoomifyOptions, value: JsObject
) {.importjs: "#.tierSizeCalculation = #".}
