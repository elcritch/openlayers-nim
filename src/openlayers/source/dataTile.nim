...
type DataTileSource* = ref object of JsRoot
proc newDataTileSource*(
  options: JsObject
): DataTileSource {.importjs: "(new olNs_source_DataTile.default(#))".}

proc handleTileChange*(
  self: DataTileSource, event: JsObject
) {.importjs: "#.handleTileChange_(#)".}

proc setTileSizes*(
  self: DataTileSource, tileSizes: JsObject
) {.importjs: "#.setTileSizes(#)".}

proc getTileSize*(
  self: DataTileSource, z: float
): JsObject {.importjs: "#.getTileSize(#)".}
...
): JsObject {.importjs: "#.getReprojTile_(#, #, #, #, #, #)".}

proc setTileGridForProjection*(
  self: DataTileSource, projection: JsObject, tilegrid: JsObject
) {.importjs: "#.setTileGridForProjection(#, #)".}
...
type DataTileSourceOptions* = ref object of JsRoot

proc newDataTileSourceOptions*(): DataTileSourceOptions {.importjs: "({})".}
proc `loader=`*(
  options: DataTileSourceOptions, value: JsObject
) {.importjs: "#.loader = #".}

proc `loader=`*(
  options: DataTileSourceOptions, value: RootRef
) {.importjs: "#.loader = #".}
...
proc `attributionsCollapsible=`*(
  options: DataTileSourceOptions, value: bool
) {.importjs: "#.attributionsCollapsible = #".}

proc `maxZoom=`*(
  options: DataTileSourceOptions, value: float
) {.importjs: "#.maxZoom = #".}
