...
type SentinelHub* = ref object of JsRoot
proc newSentinelHub*(
  options: JsObject = jsUndefined
): SentinelHub {.importjs: "(new olNs_source_SentinelHub.default(#))".}
...
proc setEvalscript*(
  self: SentinelHub, evalscript: JsObject
) {.importjs: "#.setEvalscript(#)".}
...
proc parseTokenClaims*(
  token: cstring
): JsObject {.importjs: "olNs_source_SentinelHub.parseTokenClaims(#)".}

proc getProjectionIdentifier*(
  projection: JsObject
): cstring {.importjs: "olNs_source_SentinelHub.getProjectionIdentifier(#)".}

proc serializeFunction*(
  name: cstring, funcVal: JsObject
): cstring {.importjs: "olNs_source_SentinelHub.serializeFunction(#, #)".}

type SentinelHubOptions* = ref object of JsRoot
...
proc `evalscript=`*(
  options: SentinelHubOptions, value: cstring
) {.importjs: "#.evalscript = #".}

proc `evalscript=`*(
  options: SentinelHubOptions, value: JsObject
) {.importjs: "#.evalscript = #".}

proc `evalscript=`*(
  options: SentinelHubOptions, value: RootRef
) {.importjs: "#.evalscript = #".}

proc `tileSize=`*(
  options: SentinelHubOptions, value: float
) {.importjs: "#.tileSize = #".}
...
proc `attributionsCollapsible=`*(
  options: SentinelHubOptions, value: bool
