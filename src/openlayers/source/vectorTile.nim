...
proc newVectorTile*(
  options: JsObject
): VectorTile {.importjs: "(new olNs_source_VectorTile.default(#))".}

proc getOverlaps*(self: VectorTile): bool {.importjs: "#.getOverlaps()".}
proc getSourceTiles*(
  self: VectorTile, pixelRatio: float, projection: JsObject, tile: JsObject
): JsObject {.importjs: "#.getSourceTiles(#, #, #)".}

proc removeSourceTiles*(
  self: VectorTile, tile: JsObject
) {.importjs: "#.removeSourceTiles(#)".}

proc setOverlaps*(self: VectorTile, overlaps: bool) {.importjs: "#.setOverlaps(#)".}

proc defaultLoadFunction*(
  tile: JsObject, url: cstring
) {.importjs: "olNs_source_VectorTile.defaultLoadFunction(#, #)".}

type VectorTileOptions* = ref object of JsRoot

proc newVectorTileOptions*(): VectorTileOptions {.importjs: "({})".}
proc `attributions=`*(
  options: VectorTileOptions, value: JsObject
) {.importjs: "#.attributions = #".}

proc `attributions=`*(
  options: VectorTileOptions, value: RootRef
) {.importjs: "#.attributions = #".}

proc `attributionsCollapsible=`*(
  options: VectorTileOptions, value: bool
) {.importjs: "#.attributionsCollapsible = #".}

proc `cacheSize=`*(
  options: VectorTileOptions, value: float
) {.importjs: "#.cacheSize = #".}
...
proc `urls=`*(
  options: VectorTileOptions, value: seq[cstring]
) {.importjs: "#.urls = #".}
