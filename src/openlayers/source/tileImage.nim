...
type TileImage* = ref object of JsRoot
proc newTileImage*(
  options: JsObject
): TileImage {.importjs: "(new olNs_source_TileImage.default(#))".}

proc getGutter*(self: TileImage): float {.importjs: "#.getGutter()".}
proc getTileInternal*(
  self: TileImage,
  z: float,
  x: float,
  y: float,
...
): JsObject {.importjs: "#.getTileInternal(#, #, #, #, #, #)".}

proc setRenderReprojectionEdges*(
  self: TileImage, render: bool
) {.importjs: "#.setRenderReprojectionEdges(#)".}

proc setTileGridForProjection*(
  self: TileImage, projection: JsObject, tilegrid: JsObject
) {.importjs: "#.setTileGridForProjection(#, #)".}

proc defaultTileLoadFunction*(
  imageTile: JsObject, src: cstring
) {.importjs: "olNs_source_TileImage.defaultTileLoadFunction(#, #)".}

type TileImageOptions* = ref object of JsRoot

proc newTileImageOptions*(): TileImageOptions {.importjs: "({})".}
proc `attributions=`*(
  options: TileImageOptions, value: JsObject
) {.importjs: "#.attributions = #".}

proc `attributions=`*(
  options: TileImageOptions, value: RootRef
) {.importjs: "#.attributions = #".}

proc `attributionsCollapsible=`*(
  options: TileImageOptions, value: bool
) {.importjs: "#.attributionsCollapsible = #".}

proc `cacheSize=`*(
  options: TileImageOptions, value: float
) {.importjs: "#.cacheSize = #".}

proc `crossOrigin=`*(
  options: TileImageOptions, value: cstring
