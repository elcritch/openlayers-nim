...
type UrlTile* = ref object of JsRoot
proc newUrlTile*(
  options: JsObject
): UrlTile {.importjs: "(new olNs_source_UrlTile.default(#))".}

proc tileUrlFunction*(
  self: UrlTile, tileCoord: JsObject, pixelRatio: float, projection: JsObject
): cstring {.importjs: "#.tileUrlFunction(#, #, #)".}

proc getTileLoadFunction*(
  self: UrlTile
): JsObject {.importjs: "#.getTileLoadFunction()".}
...
proc handleTileChange*(
  self: UrlTile, event: JsObject
) {.importjs: "#.handleTileChange(#)".}

proc setTileLoadFunction*(
  self: UrlTile, tileLoadFunction: JsObject
) {.importjs: "#.setTileLoadFunction(#)".}

proc setTileUrlFunction*(
  self: UrlTile, tileUrlFunction: JsObject, key: JsObject = jsUndefined
) {.importjs: "#.setTileUrlFunction(#, #)".}

proc setUrl*(self: UrlTile, url: cstring) {.importjs: "#.setUrl(#)".}
proc setUrls*(self: UrlTile, urls: seq[cstring]) {.importjs: "#.setUrls(#)".}

type UrlTileOptions* = ref object of JsRoot

proc newUrlTileOptions*(): UrlTileOptions {.importjs: "({})".}
proc `attributions=`*(
  options: UrlTileOptions, value: JsObject
) {.importjs: "#.attributions = #".}

proc `attributions=`*(
  options: UrlTileOptions, value: RootRef
) {.importjs: "#.attributions = #".}

proc `attributionsCollapsible=`*(
  options: UrlTileOptions, value: bool
) {.importjs: "#.attributionsCollapsible = #".}

proc `cacheSize=`*(
  options: UrlTileOptions, value: float
) {.importjs: "#.cacheSize = #".}
