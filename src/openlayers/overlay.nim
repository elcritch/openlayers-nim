...
type Overlay* = ref object of JsRoot
proc newOverlay*(): Overlay {.importjs: "(new olNs_Overlay.default())".}

type OverlayOptions* = ref object of JsRoot
...
proc `position=`*(
  options: OverlayOptions, value: JsObject
) {.importjs: "#.position = #".}

proc `position=`*(
  options: OverlayOptions, value: RootRef
) {.importjs: "#.position = #".}

proc `positioning=`*(
  options: OverlayOptions, value: JsObject
) {.importjs: "#.positioning = #".}
...
proc `insertFirst=`*(
  options: OverlayOptions, value: bool
) {.importjs: "#.insertFirst = #".}
...
proc `className=`*(
  options: OverlayOptions, value: cstring
) {.importjs: "#.className = #".}

proc newOverlay*(
  options: OverlayOptions
): Overlay {.importjs: "(new olNs_Overlay.default(#))".}

type PanIntoViewOptions* = ref object of JsRoot

proc newPanIntoViewOptions*(): PanIntoViewOptions {.importjs: "({})".}
proc `animation=`*(
  options: PanIntoViewOptions, value: JsObject
) {.importjs: "#.animation = #".}

proc `animation=`*(
  options: PanIntoViewOptions, value: RootRef
) {.importjs: "#.animation = #".}

proc `margin=`*(options: PanIntoViewOptions, value: float) {.importjs: "#.margin = #".}

type PanOptions* = ref object of JsRoot
