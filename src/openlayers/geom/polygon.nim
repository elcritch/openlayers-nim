...
type Polygon* = ref object of JsRoot
proc newPolygon*(
  coordinates: JsObject, layout: JsObject = jsUndefined, ends: JsObject = jsUndefined
): Polygon {.importjs: "(new olNs_geom_Polygon.default(#, #, #))".}

proc appendLinearRing*(
  self: Polygon, linearRing: JsObject
) {.importjs: "#.appendLinearRing(#)".}
...
proc getLinearRing*(
  self: Polygon, index: float
): JsObject {.importjs: "#.getLinearRing(#)".}
...
proc circular*(
  center: JsObject,
  radius: float,
  n: JsObject = jsUndefined,
  sphereRadius: JsObject = jsUndefined,
): JsObject {.importjs: "olNs_geom_Polygon.circular(#, #, #, #)".}

proc fromExtent*(
  extent: JsObject
): JsObject {.importjs: "olNs_geom_Polygon.fromExtent(#)".}

proc fromCircle*(
  circle: JsObject, sides: JsObject = jsUndefined, angle: JsObject = jsUndefined
): JsObject {.importjs: "olNs_geom_Polygon.fromCircle(#, #, #)".}

proc makeRegular*(
  polygon: JsObject, center: JsObject, radius: float, angle: JsObject = jsUndefined
) {.importjs: "olNs_geom_Polygon.makeRegular(#, #, #, #)".}
