...
type FullScreen* = ref object of JsRoot
proc newFullScreen*(
  options: JsObject = jsUndefined
): FullScreen {.importjs: "(new olNs_control_FullScreen.default(#))".}

type FullScreenOptions* = ref object of JsRoot

proc newFullScreenOptions*(): FullScreenOptions {.importjs: "({})".}
proc `className=`*(
  options: FullScreenOptions, value: cstring
) {.importjs: "#.className = #".}
...
proc `labelActive=`*(
  options: FullScreenOptions, value: cstring
) {.importjs: "#.labelActive = #".}

proc `labelActive=`*(
  options: FullScreenOptions, value: JsObject
) {.importjs: "#.labelActive = #".}

proc `labelActive=`*(
  options: FullScreenOptions, value: RootRef
) {.importjs: "#.labelActive = #".}

proc `activeClassName=`*(
  options: FullScreenOptions, value: cstring
) {.importjs: "#.activeClassName = #".}

proc `inactiveClassName=`*(
  options: FullScreenOptions, value: cstring
) {.importjs: "#.inactiveClassName = #".}
