...
type ZoomToExtent* = ref object of JsRoot
proc newZoomToExtent*(
  options: JsObject = jsUndefined
): ZoomToExtent {.importjs: "(new olNs_control_ZoomToExtent.default(#))".}

proc handleZoomToExtent*(self: ZoomToExtent) {.importjs: "#.handleZoomToExtent()".}

type ZoomToExtentOptions* = ref object of JsRoot

proc newZoomToExtentOptions*(): ZoomToExtentOptions {.importjs: "({})".}
proc `className=`*(
  options: ZoomToExtentOptions, value: cstring
) {.importjs: "#.className = #".}
...
proc `target=`*(
  options: ZoomToExtentOptions, value: JsObject
) {.importjs: "#.target = #".}

proc `target=`*(
  options: ZoomToExtentOptions, value: RootRef
) {.importjs: "#.target = #".}
