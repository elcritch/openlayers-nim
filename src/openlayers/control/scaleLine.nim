...
type ScaleLine* = ref object of JsRoot
proc newScaleLine*(
  options: JsObject = jsUndefined
): ScaleLine {.importjs: "(new olNs_control_ScaleLine.default(#))".}
...
proc createMarker*(
  self: ScaleLine, position: JsObject
): cstring {.importjs: "#.createMarker(#)".}

proc createStepText*(
  self: ScaleLine, i: float, width: float, isLast: bool, scale: float, suffix: cstring
): cstring {.importjs: "#.createStepText(#, #, #, #, #)".}

proc getScaleForResolution*(
  self: ScaleLine
): float {.importjs: "#.getScaleForResolution()".}

type ScaleLineOptions* = ref object of JsRoot

proc newScaleLineOptions*(): ScaleLineOptions {.importjs: "({})".}
proc `className=`*(
  options: ScaleLineOptions, value: cstring
) {.importjs: "#.className = #".}

proc `minWidth=`*(
  options: ScaleLineOptions, value: float
) {.importjs: "#.minWidth = #".}
