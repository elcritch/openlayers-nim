...
type
  ZoomSliderOptions* = ref object of JsRoot

proc newZoomSliderOptions*(): ZoomSliderOptions {.importjs: "({})".}
proc `className=`*(options: ZoomSliderOptions, value: cstring) {.importjs: "#.className = #".}
proc `duration=`*(options: ZoomSliderOptions, value: float) {.importjs: "#.duration = #".}
proc `render=`*(options: ZoomSliderOptions, value: JsObject) {.importjs: "#.render = #".}
...
proc `target=`*(options: ZoomSliderOptions, value: JsObject) {.importjs: "#.target = #".}
proc `target=`*(options: ZoomSliderOptions, value: RootRef) {.importjs: "#.target = #".}

proc newZoomSlider*(options: ZoomSliderOptions): ZoomSlider {.importjs: "(new olNs_control_ZoomSlider.default(#))".}
