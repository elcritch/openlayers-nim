...
type OverviewMap* = ref object of JsRoot
proc newOverviewMap*(
  options: JsObject = jsUndefined
): OverviewMap {.importjs: "(new olNs_control_OverviewMap.default(#))".}

proc getCollapsible*(self: OverviewMap): bool {.importjs: "#.getCollapsible()".}
proc setCollapsible*(
  self: OverviewMap, collapsible: bool
...
proc setRotateWithView*(
  self: OverviewMap, rotateWithView: bool
) {.importjs: "#.setRotateWithView(#)".}

proc getOverviewMap*(self: OverviewMap): JsObject {.importjs: "#.getOverviewMap()".}

type OverviewMapOptions* = ref object of JsRoot

proc newOverviewMapOptions*(): OverviewMapOptions {.importjs: "({})".}
proc `className=`*(
  options: OverviewMapOptions, value: cstring
) {.importjs: "#.className = #".}

proc `collapsed=`*(
  options: OverviewMapOptions, value: bool
) {.importjs: "#.collapsed = #".}
...
proc `collapseLabel=`*(
  options: OverviewMapOptions, value: JsObject
) {.importjs: "#.collapseLabel = #".}

proc `collapseLabel=`*(
  options: OverviewMapOptions, value: RootRef
) {.importjs: "#.collapseLabel = #".}
...
proc `layers=`*[T](
  options: OverviewMapOptions, value: seq[T]
) {.importjs: "#.layers = #".}
