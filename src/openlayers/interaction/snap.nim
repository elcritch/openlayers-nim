...
type Snap* = ref object of JsRoot
proc newSnap*(
  options: JsObject = jsUndefined
): Snap {.importjs: "(new olNs_interaction_Snap.default(#))".}

proc addFeature*(
  self: Snap, feature: JsObject, register: JsObject = jsUndefined
) {.importjs: "#.addFeature(#, #)".}

proc removeFeature*(
  self: Snap, feature: JsObject, unlisten: JsObject = jsUndefined
) {.importjs: "#.removeFeature(#, #)".}

proc snapTo*(
  self: Snap, pixel: JsObject, pixelCoordinate: JsObject, map: JsObject
): JsObject {.importjs: "#.snapTo(#, #, #)".}

type SnapOptions* = ref object of JsRoot
...
proc `intersection=`*(
  options: SnapOptions, value: bool
) {.importjs: "#.intersection = #".}

proc `pixelTolerance=`*(
  options: SnapOptions, value: float
) {.importjs: "#.pixelTolerance = #".}

proc `segmenters=`*(
  options: SnapOptions, value: JsObject
) {.importjs: "#.segmenters = #".}

proc `segmenters=`*(
  options: SnapOptions, value: RootRef
) {.importjs: "#.segmenters = #".}
