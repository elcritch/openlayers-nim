...
type Extent* = ref object of JsRoot
proc newExtent*(
  options: JsObject = jsUndefined
): Extent {.importjs: "(new olNs_interaction_Extent.default(#))".}
...
type ExtentOptions* = ref object of JsRoot

proc newExtentOptions*(): ExtentOptions {.importjs: "({})".}
proc `condition=`*(
  options: ExtentOptions, value: JsObject
) {.importjs: "#.condition = #".}

proc `condition=`*(
  options: ExtentOptions, value: RootRef
) {.importjs: "#.condition = #".}

proc `createCondition=`*(
  options: ExtentOptions, value: JsObject
) {.importjs: "#.createCondition = #".}
...
proc `pixelTolerance=`*(
  options: ExtentOptions, value: float
) {.importjs: "#.pixelTolerance = #".}

proc `pointerStyle=`*(
  options: ExtentOptions, value: JsObject
) {.importjs: "#.pointerStyle = #".}
