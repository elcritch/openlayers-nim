...
type
  KeyboardZoomOptions* = ref object of JsRoot

proc newKeyboardZoomOptions*(): KeyboardZoomOptions {.importjs: "({})".}
proc `duration=`*(options: KeyboardZoomOptions, value: float) {.importjs: "#.duration = #".}
proc `condition=`*(options: KeyboardZoomOptions, value: JsObject) {.importjs: "#.condition = #".}
proc `condition=`*(options: KeyboardZoomOptions, value: RootRef) {.importjs: "#.condition = #".}
proc `delta=`*(options: KeyboardZoomOptions, value: float) {.importjs: "#.delta = #".}

proc newKeyboardZoom*(options: KeyboardZoomOptions): KeyboardZoom {.importjs: "(new olNs_interaction_KeyboardZoom.default(#))".}
