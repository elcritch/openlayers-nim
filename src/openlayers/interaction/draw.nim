...
type Draw* = ref object of JsRoot
proc newDraw*(
  options: JsObject
): Draw {.importjs: "(new olNs_interaction_Draw.default(#))".}

proc on*(
  self: Draw, typeVal: cstring, listener: JsObject
): JsObject {.importjs: "#.on(#, #)".}

proc on*(
  self: Draw, typeVal: cstring, listener: RootRef
): JsObject {.importjs: "#.on(#, #)".}

proc on*(
  self: Draw, typeVal: cstring, listener: proc(event: JsObject)
): JsObject {.importjs: "#.on(#, #)".}

proc on*(
  self: Draw, typeVal: cstring, listener: proc()
): JsObject {.importjs: "#.on(#, #)".}
...
proc appendCoordinates*(
  self: Draw, coordinates: JsObject
) {.importjs: "#.appendCoordinates(#)".}

proc extend*(self: Draw, feature: JsObject) {.importjs: "#.extend(#)".}

proc createRegularPolygon*(
  sides: JsObject = jsUndefined, angle: JsObject = jsUndefined
): JsObject {.importjs: "olNs_interaction_Draw.createRegularPolygon(#, #)".}

proc createBox*(): JsObject {.importjs: "olNs_interaction_Draw.createBox()".}

type DrawOptions* = ref object of JsRoot
...
proc `clickTolerance=`*(
  options: DrawOptions, value: float
) {.importjs: "#.clickTolerance = #".}
...
proc `dragVertexDelay=`*(
  options: DrawOptions, value: float
) {.importjs: "#.dragVertexDelay = #".}

proc `snapTolerance=`*(
  options: DrawOptions, value: float
) {.importjs: "#.snapTolerance = #".}
...
proc `finishCondition=`*(
  options: DrawOptions, value: JsObject
) {.importjs: "#.finishCondition = #".}

proc `finishCondition=`*(
  options: DrawOptions, value: RootRef
) {.importjs: "#.finishCondition = #".}

proc `style=`*(options: DrawOptions, value: JsObject) {.importjs: "#.style = #".}
proc `style=`*(options: DrawOptions, value: RootRef) {.importjs: "#.style = #".}
proc `geometryFunction=`*(
  options: DrawOptions, value: JsObject
) {.importjs: "#.geometryFunction = #".}
...
proc `geometryName=`*(
  options: DrawOptions, value: cstring
...
proc `freehandCondition=`*(
  options: DrawOptions, value: JsObject
) {.importjs: "#.freehandCondition = #".}
