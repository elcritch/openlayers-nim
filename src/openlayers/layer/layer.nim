...
type Layer* = ref object of JsRoot
proc newLayer*(): Layer {.importjs: "(new olNs_layer_Layer.default())".}

proc inView*(
  layerState: JsObject, viewState: JsObject
): bool {.importjs: "olNs_layer_Layer.inView(#, #)".}

type LayerOptions* = ref object of JsRoot

proc newLayerOptions*(): LayerOptions {.importjs: "({})".}
proc `className=`*(
  options: LayerOptions, value: cstring
) {.importjs: "#.className = #".}
...
proc `minResolution=`*(
  options: LayerOptions, value: float
...
proc `properties=`*(
  options: LayerOptions, value: JsObject
) {.importjs: "#.properties = #".}

proc `properties=`*(
  options: LayerOptions, value: RootRef
) {.importjs: "#.properties = #".}

proc newLayer*(
  options: LayerOptions
): Layer {.importjs: "(new olNs_layer_Layer.default(#))".}
