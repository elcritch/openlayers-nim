...
type BaseVectorLayer* = ref object of JsRoot
proc newBaseVectorLayer*(): BaseVectorLayer {.
  importjs: "(new olNs_layer_BaseVector.default())"
.}

type BaseVectorLayerOptions* = ref object of JsRoot

proc newBaseVectorLayerOptions*(): BaseVectorLayerOptions {.importjs: "({})".}
proc `className=`*(
  options: BaseVectorLayerOptions, value: cstring
) {.importjs: "#.className = #".}

proc `opacity=`*(
  options: BaseVectorLayerOptions, value: float
) {.importjs: "#.opacity = #".}

proc `visible=`*(
  options: BaseVectorLayerOptions, value: bool
) {.importjs: "#.visible = #".}

proc `extent=`*(
  options: BaseVectorLayerOptions, value: JsObject
) {.importjs: "#.extent = #".}

proc `extent=`*(
  options: BaseVectorLayerOptions, value: RootRef
) {.importjs: "#.extent = #".}
...
proc newBaseVectorLayer*(
  options: BaseVectorLayerOptions
): BaseVectorLayer {.importjs: "(new olNs_layer_BaseVector.default(#))".}
