...
type VectorTileLayer* = ref object of JsRoot
proc newVectorTileLayer*(
  options: JsObject = jsUndefined
): VectorTileLayer {.importjs: "(new olNs_layer_VectorTile.default(#))".}

proc getFeaturesInExtent*(
  self: VectorTileLayer, extent: JsObject
): JsObject {.importjs: "#.getFeaturesInExtent(#)".}
...
proc getUseInterimTilesOnError*(
  self: VectorTileLayer
): bool {.importjs: "#.getUseInterimTilesOnError()".}

proc setPreload*(self: VectorTileLayer, preload: float) {.importjs: "#.setPreload(#)".}
proc setUseInterimTilesOnError*(
  self: VectorTileLayer, useInterimTilesOnError: bool
) {.importjs: "#.setUseInterimTilesOnError(#)".}

type VectorTileLayerOptions* = ref object of JsRoot

proc newVectorTileLayerOptions*(): VectorTileLayerOptions {.importjs: "({})".}
proc `className=`*(
  options: VectorTileLayerOptions, value: cstring
) {.importjs: "#.className = #".}

proc `opacity=`*(
  options: VectorTileLayerOptions, value: float
) {.importjs: "#.opacity = #".}

proc `visible=`*(
  options: VectorTileLayerOptions, value: bool
) {.importjs: "#.visible = #".}

proc `extent=`*(
  options: VectorTileLayerOptions, value: JsObject
) {.importjs: "#.extent = #".}

proc `extent=`*(
  options: VectorTileLayerOptions, value: RootRef
) {.importjs: "#.extent = #".}
