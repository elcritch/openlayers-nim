...
type RegularShape* = ref object of JsRoot
proc newRegularShape*(
  options: JsObject
): RegularShape {.importjs: "(new olNs_style_RegularShape.default(#))".}
...
proc createRenderOptions*(
  self: RegularShape
): JsObject {.importjs: "#.createRenderOptions()".}

proc render*(self: RegularShape) {.importjs: "#.render()".}

type RegularShapeOptions* = ref object of JsRoot
...
proc `radius2=`*(
  options: RegularShapeOptions, value: float
) {.importjs: "#.radius2 = #".}

proc `angle=`*(options: RegularShapeOptions, value: float) {.importjs: "#.angle = #".}
proc `displacement=`*(
  options: RegularShapeOptions, value: seq[float]
) {.importjs: "#.displacement = #".}

proc `stroke=`*(
  options: RegularShapeOptions, value: JsObject
) {.importjs: "#.stroke = #".}

proc `stroke=`*(
  options: RegularShapeOptions, value: RootRef
) {.importjs: "#.stroke = #".}
...
proc `rotateWithView=`*(
  options: RegularShapeOptions, value: bool
...
type RenderOptions* = ref object of JsRoot

proc newRenderOptions*(): RenderOptions {.importjs: "({})".}
proc `strokeStyle=`*(
  options: RenderOptions, value: JsObject
) {.importjs: "#.strokeStyle = #".}

proc `strokeStyle=`*(
  options: RenderOptions, value: RootRef
) {.importjs: "#.strokeStyle = #".}

proc `strokeWidth=`*(
  options: RenderOptions, value: float
) {.importjs: "#.strokeWidth = #".}
...
proc `lineDash=`*(
  options: RenderOptions, value: seq[float]
) {.importjs: "#.lineDash = #".}
