...
type IconImageCache* = ref object of JsRoot
proc newIconImageCache*(): IconImageCache {.
  importjs: "(new olNs_style_IconImageCache.default())"
.}
...
proc get*(
  self: IconImageCache, src: cstring, color: JsObject
): JsObject {.importjs: "#.get(#, #)".}

proc getPattern*(
  self: IconImageCache, src: cstring, color: JsObject
): JsObject {.importjs: "#.getPattern(#, #)".}
...
proc set*(
  ...
  color: JsObject,
  iconImage: JsObject,
  pattern: JsObject = jsUndefined,
) {.importjs: "#.set(#, #, #, #)".}

proc setSize*(self: IconImageCache, maxCacheSize: float) {.importjs: "#.setSize(#)".}

proc getCacheKey*(
  src: cstring, color: JsObject
): cstring {.importjs: "olNs_style_IconImageCache.getCacheKey(#, #)".}
